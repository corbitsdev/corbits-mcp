...
// tokens so refresh keeps working after the login completes.
//
...
export async function discoverMcpLoginEntry(
  ...
  if (metadata.registration_endpoint !== undefined)
    authorizationServer.registrationEndpoint = metadata.registration_endpoint;
  if (metadata.grant_types_supported !== undefined)
    authorizationServer.grantTypesSupported = metadata.grant_types_supported;
  if (metadata.scopes_supported !== undefined)
    authorizationServer.scopesSupported = metadata.scopes_supported;
  ...
  if (metadata.code_challenge_methods_supported !== undefined)
    authorizationServer.codeChallengeMethodsSupported =
      metadata.code_challenge_methods_supported;

  const entry: McpLoginEntry = { resourceUrl, authorizationServer };
  if (resourceScopesSupported !== undefined)
    entry.resourceScopesSupported = resourceScopesSupported;
  return entry;
}
...
export async function registerMcpClient(
  ...
  if (scope !== undefined) registration.scope = scope;
  return registration;
}
...
export function mcpClientConfig(
  ...
  const scopeOptions: SelectMcpScopesOptions = {
    entry,
  ...
  return {
    ...
    tokenTimeoutMs: opts.tokenTimeoutMs ?? DEFAULT_TOKEN_TIMEOUT_MS,
  };
}
